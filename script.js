...
function processData() {
  ...
  if (currentAlgorithm === "binary") {
    ...
    // Jalankan binary search dengan visualisasi
    const resultIndex = binarySearchWithVisualization(sortedData, searchValue);
    ...
    playAnimation();
        ...
        if (currentAlgorithm === "bubble") {
            executionTime = bubbleSortWithSteps(sortedData);
        } else if (currentAlgorithm === "shell") {
            executionTime = shellSortWithSteps(sortedData);
        }
        ...
        // Tampilkan pesan hasil dengan waktu eksekusi
        ...
        resultText.innerHTML = `
            ...
            <p>Waktu eksekusi: <strong>${executionTime.toFixed(2)} ms</strong></p>
        `;
...
function bubbleSortWithSteps(arr) {
    ...
    return endTime - startTime; // Return waktu eksekusi dalam milidetik
}
...
function binarySearch(arr, target) {
  ...
  while (left <= right) {
    ...
    steps.push(
      `Membagi data: indeks ${left}-${right}, titik tengah ${mid} (${arr[mid].name}: ${arr[mid].score})`
    );
    ...
    if (arr[mid].score === target) {
      ...
      steps.push(
        `${target} lebih besar dari ${arr[mid].score}, mencari di bagian kanan`
      );
      left = mid + 1;
    } else {
      steps.push(
        `${target} lebih kecil dari ${arr[mid].score}, mencari di bagian kiri`
      );
      right = mid - 1;
    }
  }
...
// Bubble Sort dengan perhitungan waktu
function bubbleSortWithSteps(arr) {
...
// Shell Sort dengan perhitungan waktu
function shellSortWithSteps(arr) {
    const startTime = performance.now();
    let n = arr.length;
...
// Binary Search dengan perhitungan waktu
